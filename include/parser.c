...
void checkFolderExists(char* _folderPath){
    ...
    fprintf (stdout, "ERROR: Selected folder path is not valid (folder doesn't exit / bad folder path)\n");
    exit(1);
}
...
char* parseUploadFilePath(char* _filePath){
    ...
    fprintf(stdout, "ERROR: Selected file path is not accesable or doesn't exist: %s\n", _filePath);
    exit(1);
}
...
char* parseFDownloadilePath(char* _filePath){
    if (access(_filePath, F_OK) != 0) return _filePath;
    fprintf(stdout, "ERROR: Selected file already exists, can't rewrite: %s\n", _filePath);
    exit(1);
}
